...
#get sequences from alignment into dict format
#with open('/nesi/nobackup/uc04105/new_databases_May/final_tree_set/CPA_full_tree_aligned.faa', 'a') as C:
  #  count = 0
  #  for record in SeqIO.parse('/nesi/nobackup/uc04105/new_databases_May/final_tree_set/SG_seed_update_jan142024_135seq_PF00999aligned.fasta.fasta', 'fasta'):
   #     prot_id = '>' + record.id.split('_')[-1] + '_' + record.id.split('_')[-2] + '_' + str(count) + '\n'
   #     seq = str(record.seq)
   #     fasta = prot_id + seq + '\n'
   #     count = count + 1
   #     C#.write(fasta)
   # for record in SeqIO.parse('/nesi/nobackup/uc04105/new_databases_May/final_tree_set/Bacteria_allhmmscanned_final_clustered_at0.7.fasta_rep_seq.fasta', 'fasta'):
   #     prot_id = '>' + record.id.split('_tax:')[0]
   #     seq = str(record.seq)
   #     fasta = prot_id + '\n' + seq + '\n'
        #C.write(fasta)
 ##   for record in SeqIO.parse('/nesi/nobackup/uc04105/new_databases_May/final_tree_set/Archaea_allhmmscanned_final_clustered_at0.7.fasta_rep_seq.fasta', 'fasta'):
   #     prot_id = '>' + record.id.split('_tax:')[0]
   #     seq = str(record.seq)
     #   fasta = prot_id + '\n' + seq + '\n'
        #C.write(fasta)
  #  for record in SeqIO.parse('/nesi/nobackup/uc04105/new_databases_May/final_tree_set/Eukaraya_all_hmmscanned_aligned_treeinput_clusterd_at_0.7.faa_rep_seq.fasta', 'fasta'):
  #      prot_id = '>' + record.id.split('_tax:')[0]
    ##    seq = str(record.seq)
    #    fasta = prot_id + '\n' + seq + '\n'
        #C.write(fasta)
