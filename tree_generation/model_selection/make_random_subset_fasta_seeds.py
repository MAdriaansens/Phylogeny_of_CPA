...
CPA_id_list= []
HMMaligned = '/nesi/nobackup/uc04105/fasta_files/Seed_NhaA/SG_seed_update_jan142024_135seq_alignedPF009999_filtered.faa.fasta'
...
def return_fasta_from_list(HMMaligned, random_list, count, outdir):
    ...
    with open('{}/Seed_HMMscanned_PF00999_aligned_randomset_{}.fasta'.format(outdir, count), 'a') as OUT:
        for record in SeqIO.parse(HMMaligned, 'fasta'):
            if record.id.split('|')[1] not in random_list:
                pass
            else:
