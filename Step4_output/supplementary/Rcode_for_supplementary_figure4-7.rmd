...
IT_Eukarya <- read.delim('~/Antiporter_Eukarya_21April2026.tsv', sep='\t', stringsAsFactors = TRUE)
sum(IT_Eukarya$CPA_count)


TypesE<- read.delim('~/CPA_types_Eukarya_5may.tsv', sep='\t', stringsAsFactors = TRUE)
...
IT_Eukarya <- IT_Eukarya[-c(246:248),]
 #species with no CPA off course have no CPA types present so here I edit that in. 
#write it out
#write.table(IT_Eukarya, 'Antiporter_Eukarya_30Apri.tsv.tsv', sep='\t', row.names = FALSE)
```
...
```{r load_Archaea}
#these are the files containing all the antiporters, in our architecture it is called:
IT_Archaea <- read.delim('~/Antiporter_Archaea_21April.tsv', sep='\t', stringsAsFactors = TRUE)

sum(IT_Archaea$CPA_count)
sum(IT_Archaea$NhaC_binary)
TypesA <- read.delim('~/CPA_types_Archaea_5may.tsv', sep='\t', stringsAsFactors = TRUE)
...
#rm(TypesA)
write.table(IT_Archaea, file='Antiporter_Archaea_30Apri.tsv', quote=FALSE, sep=',')
...
GTDB_Arc <- read.delim('~/ar53_metadata.tsv', sep='\t', stringsAsFactors = TRUE, row.names = NULL)
...
```{r load_Bacteria}
#these are the files containing all the antiporters, in our architecture it is called:
IT_Bacteria <- read.delim('~/Antiporter_Bacteria_25April.tsv', sep='\t', stringsAsFactors = TRUE)

TypesB <- read.delim('~/CPA_types_Bacteria_5may.tsv', sep='\t', stringsAsFactors = TRUE)
...
```{r}
GTDB_Bac <- read.delim('~/bac120_metadata_r226.tsv', sep='\t', stringsAsFactors = TRUE, row.names = NULL)
...
rm(TypesB)
write.table(IT_Bacteria, file='Antiporter_Bacteria_30Apri.tsv', quote=FALSE, sep=',')
