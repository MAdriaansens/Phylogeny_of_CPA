...
IT_Eukarya <- read.delim('/nesi/nobackup/uc04105/new_databases_May/Euk_database_May/Antiporter_Eukarya_21April2026.tsv', sep='\t', stringsAsFactors = TRUE)
sum(IT_Eukarya$CPA_count)


TypesE<- read.delim('/nesi/nobackup/uc04105/new_databases_May/final_april28/CPA_types_Eukarya_5may.tsv', sep='\t', stringsAsFactors = TRUE)
TypesE <- rename(TypesE, species_name = GTDB_id)
TypesE <- rename(TypesE, Uncharacterized = Uncharacaterized)
...
median(TypesE$CPA1)
median(TypesE$Kef)
median(TypesE$CPA1_SL)

TypesE<-TypesE %>% distinct()
IT_Eukarya <- merge(IT_Eukarya, TypesE, by ='species_name', all =TRUE)
IT_Eukarya [is.na(IT_Eukarya)] <- 0
IT_Eukarya <- IT_Eukarya[-c(246:248),]
 #species with no CPA off course have no CPA types present so here I edit that in. 
#write it out
write.table(IT_Eukarya, 'Antiporter_Eukarya.tsv', sep='\t', row.names = FALSE)
```
...
mean(Major_tax_CPAobservations$percentage)
sd(Major_tax_CPAobservations$percentage)/sqrt(length(Major_tax_CPAobservations$percentage))

mean(Major_tax_NhaCobservations$percentage)
sd(Major_tax_NhaCobservations$percentage)/sqrt(length(Major_tax_NhaCobservations$percentage))

mean(Major_tax_NhaDobservations$percentage)
sd(Major_tax_NhaDobservations$percentage)/sqrt(length(Major_tax_NhaDobservations$percentage))

mean(Major_tax_NhaBobservations$percentage)
sd(Major_tax_NhaBobservations$percentage)/sqrt(length(Major_tax_NhaBobservations$percentage))
...
MeanEuk <-  mean(IT_Eukarya$Antiporter_count[IT_Eukarya$Antiporter_binary==1] )
...
```{r load_Archaea}
#these are the files containing all the antiporters, in our architecture it is called:
IT_Archaea <- read.delim('/nesi/nobackup/uc04105/new_databases_May/GTDB_226/Antiporter_Archaea_21April.tsv', sep='\t', stringsAsFactors = TRUE)

sum(IT_Archaea$CPA_count)
sum(IT_Archaea$NhaC_binary)
TypesA <- read.delim('/nesi/nobackup/uc04105/new_databases_May/final_april28/CPA_types_Archaea_5may.tsv', sep='\t', stringsAsFactors = TRUE)
...
#rm(TypesA)
write.table(IT_Archaea, file='Antiporter_Archaea.tsv', quote=FALSE, sep=',')
...
#hist(IT_Archaea_nonna$antiporter_per_megabases,  breaks = 100) + abline(v = 1.93, col = "blue", lwd = 2, lty = 2)
...
Phyla_Ac_CPAobservations <- IT_Archaea_clean[IT_Archaea_clean$CPA_binary == 1,] %>%count(Phyla)
Phyla_Ac_CPAobservations<- merge(Phyla_Ac_observations, Phyla_Ac_CPAobservations, Phyla = 'Phyla')
Phyla_Ac_CPAobservations$percentage <-  (Phyla_Ac_CPAobservations$n/Phyla_Ac_CPAobservations$Species_total)*100

mean(Phyla_Ac_CPAobservations$percentage)
sd(Phyla_Ac_CPAobservations$percentage)/sqrt(length(Phyla_Ac_CPAobservations$percentage))
...
mean(Phyla_Ac_NhaCobservations$percentage)
sd(Phyla_Ac_NhaCobservations$percentage)/sqrt(length(Phyla_Ac_NhaCobservations$percentage))

mean(Phyla_Ac_NhaDobservations$percentage)
sd(Phyla_Ac_NhaDobservations$percentage)/sqrt(length(Phyla_Ac_NhaDobservations$percentage))
...
Both_AONLY_SPECIES <- IT_Archaea[IT_Archaea$IT_count >0 & IT_Archaea$CPA_count > 0 & IT_Archaea$Phyla %in% Phyla_Ac_observations$Phyla,]
...
```{r, complete_cpa_only}
CPA_AONLY_SPECIES <- IT_Archaea[IT_Archaea$IT_count == '0' & IT_Archaea$CPA_count > 0 & IT_Archaea$Phyla %in% Phyla_Ac_observations$Phyla,]
...
Missing_N_list = list(c())
Missing_gene_count = list(c())
for (taxa in Phyla_Ac_observations$Phyla) {
...
IT_AONLY_SPECIES <- IT_Archaea[IT_Archaea$IT_count >0 & IT_Archaea$CPA_count=='0' & IT_Archaea$Phyla %in% Phyla_Ac_observations$Phyla,]
...
#everything has at least one both
...
Altia <- c('Altiarchaeota', 0)
Asgard <- c('Asgardarchaeota', 0)
Kor<- c('Korarchaeota', 0)
SpSt<- c('SpSt-1190', 0)
EX448452 <- c('EX4484-52', 0)
Methanobacteriota_B<- c('Methanobacteriota_B', 0)
Nanohalarchaeota<- c('Nanohalarchaeota', 0)
Undinarchaeota <- c('Undinarchaeota', 0)
B1Sed10 <- c('B1Sed10-29', 0)



no_IT_butCPA <- rbind(no_IT_butCPA, Undinarchaeota)
no_IT_butCPA <- rbind(no_IT_butCPA, Asgard)
no_IT_butCPA <- rbind(no_IT_butCPA, Altia)
no_IT_butCPA <- rbind(no_IT_butCPA, Kor)
no_IT_butCPA <- rbind(no_IT_butCPA, SpSt)
...
no_CPA_but_IT <- rbind(no_CPA_but_IT, Nanohalarchaeota, Altia, SpSt, Undinarchaeota, EX448452, Methanobacteriota_B, B1Sed10 )
...
PERA<-ggplot(pivot_Arc, aes(fill=status, y=Count, x=Phyla)) + geom_bar(position = 'dodge', stat='identity') + theme_minimal()  + scale_fill_manual(values = c( 'deepskyblue1', 'darkviolet','brown1')) + scale_alpha_manual(values = c(0.3, 0.9, 0.5)) +
  theme(axis.text.x = element_text(color = "black", size = 36, angle = 90, hjust = .5, vjust = .5, face = "italic"),
        ...
        axis.title.y = element_text(color = "black", size = 25, angle = 0, hjust = .5, vjust = .5, face = "bold")) + geom_vline(xintercept = c(1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5,13.5,14.5,15.5,16.5,17.5,18.5)) + geom_hline(yintercept = mean(IT_Archaea_nonna$Antiporter_count), lty='dashed') +
...
```{r load_Bacteria}
#these are the files containing all the antiporters, in our architecture it is called:
IT_Bacteria <- read.delim('/nesi/nobackup/uc04105/new_databases_May/GTDB_226/Antiporter_Bacteria_25April.tsv', sep='\t', stringsAsFactors = TRUE)

TypesB <- read.delim('/nesi/nobackup/uc04105/new_databases_May/final_april28/CPA_types_Bacteria_5may.tsv', sep='\t', stringsAsFactors = TRUE)
TypesB <-TypesB %>% distinct()

TypesB <- TypesB %>% rename( "GTDB_tax" = GTDB_id)
...
rm(TypesB)
write.table(IT_Bacteria, file='Antiporter_Bacteria.tsv', quote=FALSE, sep=',')
...
Phyla_Bc_NhaBobservations <- IT_Bacteria_clean[IT_Bacteria_clean$NhaB_binary == 1,] %>%count(Phyla)
Phyla_Bc_NhaBobservations<- merge(Phyla_Bc_observations, Phyla_Bc_NhaBobservations, Phyla = 'Phyla')
Phyla_Bc_NhaBobservations$percentage <-  (Phyla_Bc_NhaBobservations$n/Phyla_Bc_NhaBobservations$Species_total)*100
...
Phyla_Bc_CPAobservations <- IT_Bacteria_clean[IT_Bacteria_clean$CPA_binary == 1,] %>%count(Phyla)
Phyla_Bc_CPAobservations<- merge(Phyla_Bc_observations, Phyla_Bc_CPAobservations, Phyla = 'Phyla')
Phyla_Bc_CPAobservations$percentage <-  (Phyla_Bc_CPAobservations$n/Phyla_Bc_CPAobservations$Species_total)*100

mean(Phyla_Bc_CPAobservations$percentage)
sd(Phyla_Bc_CPAobservations$percentage)/sqrt(length(Phyla_Bc_CPAobservations$percentage))

mean(Phyla_Bc_NhaBobservations$percentage)
sd(Phyla_Bc_NhaBobservations$percentage)/sqrt(length(Phyla_Bc_NhaBobservations$percentage))

 
 

mean(Phyla_Bc_NhaCobservations$percentage)
sd(Phyla_Bc_NhaCobservations$percentage)/sqrt(length(Phyla_Bc_NhaCobservations$percentage))

mean(Phyla_Bc_NhaDobservations$percentage)
sd(Phyla_Bc_NhaDobservations$percentage)/sqrt(length(Phyla_Bc_NhaDobservations$percentage))
