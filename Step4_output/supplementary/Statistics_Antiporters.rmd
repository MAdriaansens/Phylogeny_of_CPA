...
```{r, Load Eukarya data and wrangel it a bit2}
library(tidyverse)
library(dplyr)
...
```{r, linear regression model of factors3}
library(dplyr)
...
#CPA count and IT count are posively correlated, with R adjusted of 0.658
```
...
```{r Generate boxplot using cleaned Archaea set10}
...
```{r Generate boxplot using cleaned Bacteria set15}
